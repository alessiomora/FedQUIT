...
from federated_fedquit.baselines_utility.mode_unlearning import mode_unlearning_round
...
from federated_fedquit.utility import create_model, get_test_dataset, preprocess_ds, preprocess_ds_test, list_clients_to_string, save_line_to_file, find_last_checkpoint
...
def main(cfg: DictConfig) -> None:
    ...
    if resume_training:
        ...
        if algorithm not in ["natural"]:
            ...
            if dataset not in ["cifar100-transformer", "birds-transformer"]:
            ...
            # -------------------------------
            print("[Retrained] Retain acc")
            _, _ = model_retrained.evaluate(ds_retain, verbose=2)
            print("[Server - Resumed] Retain acc ")
            _, _ = server_model.evaluate(ds_retain, verbose=2)
        # ----------------
        ...
        if algorithm not in ["natural"] and test_acc >= test_acc_retrained:
            ...
            if resumed_round == SAVE_ROUND_CLIENTS:
                if resume_training:
                    if algorithm not in ["pseudo_gradient_ascent_single", "pseudo_gradient_ascent", "mode"]:
    ...
    if not early_stop_recovery:
        for r in range(resumed_round + 1, resumed_round + total_rounds + 1):
            if algorithm not in ["mode"] or max_rounds < r:
                ...
                if r == r_first_recovery +1:
                    ...
                    degradation_model = create_model(dataset=dataset, total_classes=total_classes)
                else: # load degradation model
                    print("[MoDE] Loading degradation model...")
                    degradation_model = create_model(dataset=dataset, total_classes=total_classes)
                    degradation_model.load_weights(model_checkpoint_dir_de)

                learning_rates_mode = [0.1 if model in ["ResNet18"] else 3e-4, learning_rate_guidance]
                ...
                print(f"[MoDE] Saving degradation model at {model_checkpoint_dir_de}...")
            ...
            if save_checkpoint == "save_last":
                ...
                if r % checkpoint_frequency == 0:
                    ...
                    if resume_training and algorithm not in ["natural", "projected_ga", ]:
                        shutil.rmtree(dir, ignore_errors=True)
                    print("Saving checkpoint global model......")
