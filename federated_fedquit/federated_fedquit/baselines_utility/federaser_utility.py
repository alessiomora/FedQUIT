...
from federated_fedquit.dataset import load_selected_client_statistics, \
    load_client_datasets_from_files
from federated_fedquit.utility import preprocess_ds, create_model


def client_update(cid, local_model, model, t=1, epochs=1, total_clients=10, alpha=-1, dataset="cifar-100", local_batch_size=32,
                  learning_rate=0.1, lr_decay=0.998):
    ...
    if model in ["ResNet18"]:
        optimizer = tf.keras.optimizers.experimental.SGD(
            learning_rate=learning_rate * (lr_decay ** t),
    ...
    if epochs<1:
        print(f"Dataset batches: {len(ds_train_client)}")
        print(f"Ecali: {epochs}, taking the fraction")
        fraction = int(epochs*len(ds_train_client))
        ds_train_client = ds_train_client.take(fraction)
        print(f"Dataset batches: {len(ds_train_client)}")
        epochs=1
...
class DiskFedEraser:
    def __init__(self, forget_client_id, alpha, dataset_name, model_name, learning_rate, initial_model_weights, delta_t=2, Ecali=1, total_classes=100, updates_dir=os.path.join("federaser", "updates_dir"), save_dir=os.path.join("federaser", "state"),):
        """
        forget_client_id: ID of the client to forget
        updates_dir: base path where original updates are stored as updates_dir/round_{t}/client_{id}.npz
        save_dir: path to save federaser internal state
        """
        ...
        os.makedirs(os.path.join(save_dir, f"client_{forget_client_id}"), exist_ok=True)
        self.state_path = os.path.join(save_dir, f"client_{forget_client_id}", "state.json")
        self.weights_path = os.path.join(save_dir, f"client_{forget_client_id}", "model_weights.npz")

        self.model = create_model(dataset=dataset_name, total_classes=total_classes)


        if os.path.exists(self.state_path) and os.path.exists(self.weights_path):
            self._load_state()
        else:
            self.round_counter = 0
            self.retained_rounds = []
            self.model.set_weights(initial_model_weights)

    def _save_state(self):
        np.savez(self.weights_path, *self.model.get_weights())
    ...
    def _load_state(self):
        ...
        self.model.set_weights([weights[f'arr_{i}'] for i in range(len(weights.files))])
        with open(self.state_path, "r") as f:
            state = json.load(f)
            self.round_counter = state["round_counter"]
            self.retained_rounds = state["retained_rounds"]

    def calibration_training(self, client_id):
        print("Calibration training..")

        # Clone the global clean model before calibration
        fresh_model = create_model(dataset=self.dataset_name, total_classes=self.total_classes)
        fresh_model.set_weights(self.model.get_weights())

        original_weights = fresh_model.get_weights()

        # original_weights = model.get_weights()
        # model.fit(dataset, epochs=epochs, verbose=0)
        effective_round_index = len(self.retained_rounds)

        _, amount_of_data = client_update(client_id, fresh_model, model=self.model_name,
                                    t=effective_round_index, epochs=self.Ecali,
                                    total_clients=10, alpha=self.alpha,
                                    dataset=self.dataset_name,
                                    learning_rate=self.learning_rate)

        new_weights = fresh_model.get_weights()
        # return the delta
        return [new - orig for new, orig in zip(new_weights, original_weights)], amount_of_data

    def calibrate_update(self, original_update, fresh_update):
        stored_norm = np.sqrt(sum(np.sum(np.square(u)) for u in original_update))
        fresh_norm = np.sqrt(sum(np.sum(np.square(u)) for u in fresh_update))
        scale = stored_norm / (fresh_norm + 1e-12)
        return [scale * u for u in fresh_update]

    def aggregate_updates(self, calibrated_updates, client_weights):
        total_weight = sum(client_weights)
        aggregated = []
        for layer_updates in zip(*calibrated_updates):
    ...
    def apply_update(self, update):
        ...
        new_weights = [w + u for w, u in zip(current_weights, update)]
        self.model.set_weights(new_weights)

    def _load_client_update(self, round_idx, client_id):
        path = os.path.join(self.updates_dir, f"round_{round_idx:03d}", f"client_{client_id}.npz")

        if not os.path.exists(path):
            raise FileNotFoundError(f"Missing update file for client {client_id} in round {round_idx}")
        data = np.load(path, allow_pickle=True)
        return [data[f'arr_{i}'] for i in range(len(data.files))]

    def update(self, round_idx, total_clients):
        """
        Called at the end of a round.
        Args:
            round_idx (int): current round
        """
        self.round_counter = round_idx

        if round_idx % self.delta_t != 0 and round_idx != 1:
            print("[FedEraser] Not a retained round, skipping..")
            self._save_state()
            return

        # Use the current corrected model (w_{t-1}') as the reference
        reference_weights = self.model.get_weights()

        self.retained_rounds.append(round_idx)

        calibrated_updates = []
        client_weights = []

        for client_id in range(total_clients):
            print(f"[FedEraser] Processing client {client_id}...")
            if client_id == self.forget_client_id:
                print("[FedEraser] Skipping forgotten client.")
                continue

            # Load the original (pre-unlearning) update from disk
            original_update = self._load_client_update(round_idx, client_id)

            # Reset model to w_{t-1}' for calibration training
            self.model.set_weights(reference_weights)

            # Get calibrated update
            fresh_update, client_data_size = self.calibration_training(client_id)

            if round_idx == 1:
                print("[FedEraser] First round — using unscaled fresh update.")
                calibrated = original_update
            else:
                calibrated = self.calibrate_update(original_update, fresh_update)

            # calibrated = self.calibrate_update(original_update, fresh_update)

            # Overwrite and collect for aggregation
            self._overwrite_client_update(round_idx, client_id, calibrated)
            calibrated_updates.append(calibrated)
            client_weights.append(client_data_size)

        aggregated_update = self.aggregate_updates(calibrated_updates, client_weights)

        # Apply to w_{t-1}' to get w_t'
        self.apply_update(aggregated_update)

        self._save_state()

    def _overwrite_client_update(self, round_idx, client_id, new_update):
        path = os.path.join(self.updates_dir, f"round_{round_idx:03d}",
                            f"client_{client_id}.npz")

        os.makedirs(os.path.dirname(path), exist_ok=True)
        np.savez(path, *new_update)
    ...
    #
    #     if round_idx == 1:
    #         print("[FedEraser] Saving initial global model..")
    #         self._save_state()
    #         return
    #
    #     if round_idx % self.delta_t != 0:
    #         print("[FedEraser] Not retained round, skipping..")
    #         self._save_state()
    #         return
    #
    #     # Load w_{t-1} explicitly before calibration
    #     try:
    #         reference_weights = self._load_model_weights_from_file(round_idx - 1)
    #         self.model.set_weights(reference_weights)
    #     except FileNotFoundError:
    #         print(f"[FedEraser] WARNING: Could not load global model from round {round_idx - 1}, using in-memory weights.")
    #
    #     self.retained_rounds.append(round_idx)
    #
    #     calibrated_updates = []
    #     client_weights = []
    #
    #     for client_id in range(0, total_clients):
    #         print(f"[FedEraser] Loop, now client: {client_id}")
    #         if client_id == self.forget_client_id:
    #             print("[FedEraser] Skipping unlearning client..")
    #             continue
    #
    #         # Load original update from disk
    #         original_update = self._load_client_update(round_idx, client_id)
    #
    #         # Reset model to w_{t-1} for each calibration
    #         self.model.set_weights(reference_weights)
    #
    #         # Calibrate update with fresh training
    #         fresh_update, client_data_size = self.calibration_training(client_id)
    #         calibrated = self.calibrate_update(original_update, fresh_update)
    #
    #         # Overwrite update and collect for aggregation
    #         self._overwrite_client_update(round_idx, client_id, calibrated)
    #         calibrated_updates.append(calibrated)
    #         client_weights.append(client_data_size)
    #
    ...
    #     Args:
    #         round_idx (int): current round
    #         client_ids (list of str): client IDs that participated in this round
    #         client_data_dict (dict): {client_id: dataset}, for calibration
    #     """
    #     self.round_counter = round_idx
    #
    #
    ...
    #             continue
    #
    #         # Load update from disk
    #         original_update = self._load_client_update(round_idx, client_id)
    #
    #         # Run calibration
    #         fresh_update, client_amount_of_data = self.calibration_training(client_id)
    #
    #         # Calibrate and overwrite
    #         calibrated = self.calibrate_update(original_update, fresh_update)
    #         self._overwrite_client_update(round_idx, client_id, calibrated)
    #
    #         calibrated_updates.append(calibrated)
    #         client_weights.append(client_amount_of_data)
    #
    #     aggregated_update = self.aggregate_updates(calibrated_updates, client_weights)
    #     self.apply_update(aggregated_update)
    #
    ...
    def _load_model_weights_from_file(self, round_idx_minus_1):
        path = os.path.join('federaser', 'global_model', )
        path = os.path.join(path, f"global_model_{round_idx_minus_1}.npz")
...
#
#
# def federated_learning_unlearning(init_global_model, client_loaders, test_loader,
#                                   FL_params):
#     # all_global_models, all_client_models 为保存起来所有的old FL models
#     print(5 * "#" + "  Federated Learning Start" + 5 * "#")
#     std_time = time.time()
#     old_GMs, old_CMs = FL_Train(init_global_model, client_loaders, test_loader,
#                                 FL_params)
#     end_time = time.time()
...
#     if (
#             FL_params.unlearn_interval == 0 or FL_params.unlearn_interval > FL_params.global_epoch):
#         raise ValueError(
...
#         temp.pop(
#             forget_client)  # During Unlearn, the model saved by the forgotten user pops up
...
#
#         new_client_models = global_train_once(global_model, client_data_loaders,
#                                               test_loader, FL_params)
#
#         new_GM = unlearning_step_once(selected_CMs[epoch], new_client_models,
#                                       selected_GMs[epoch + 1], global_model)
#
...
#
#
# def unlearning_step_once(old_client_models, new_client_models,
...
#
#         old_param_update[layer] = old_param_update[layer] - \
#                                   global_model_before_forget.state_dict()[
#                                       layer]  # 参数： oldCM - oldGM_t
#         new_param_update[layer] = new_param_update[layer] - \
#                                   global_model_after_forget.state_dict()[
...
#
#         return_model_state[layer] = new_global_model_state[
#                                         layer] + step_length * step_direction
#
...
#
#             old_param_update[layer] = old_param_update[layer] - \
#                                       old_global_model.state_dict()[
#                                           layer]  # 参数： oldCM_t - oldGM_t
#
#             return_model_state[layer] = current_global_model.state_dict()[layer] + \
#                                         old_param_update[
