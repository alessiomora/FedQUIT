...
#     data_loader = torch.utils.data.DataLoader(data_loader.dataset, batch_size=1, shuffle=False, num_workers = 0, prefetch_factor = 10)
#     prob = []
#     with torch.no_grad():
...
#     return torch.cat(prob), np.concatenate([y for _, y in dataset], axis=0)
...
def SVC_MIA(shadow_train, target_train, target_test, shadow_test, model):
    ...
    # print(m)
    # return m
    print(f"MIA confidence based {acc_conf}")
    return acc_conf
